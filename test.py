...
cfg.PATHS['working_dir'] = utils.gettempdir(dirname='/home/nirab/OGGMOGGM/OGGM-GettingStarted', reset=True)
...
for ax, border in zip(np.array(axs).flatten(), [10, 80, 160, 250]):
    gdirs = workflow.init_glacier_regions(rgi_ids, from_prepro_level=2, prepro_border=border)

    # Plotting the Glacier Direcory
    graphics.plot_domain(gdirs, ax=ax, title='Border: {}'.format(border),
    add_colorbar=False, lonlat_contours_kwargs={'add_tick_labels':False})
    print("Printing Plot.....")
    plt.tight_layout()
